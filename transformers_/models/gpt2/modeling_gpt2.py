...
from ....utils import quantConfig, quantize_kv_per_token
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None, quant_config:quantConfig=None):
        super().__init__()
    ...
    def forward(
        ...
        **kwargs,
        ...
        # During training: quantize entire K/V tensors with gradient flow via STE
        # During generation: quantize current token's K/V before caching
        if self.quant_config is not None and self.quant_config.KV_bit < 16:
            key_states = quantize_kv_per_token(
                key_states,
                num_bits=self.quant_config.KV_bit,
                clip_val=self.quant_config.gradclip if self.quant_config.gradclip != (0, 0) else None,
                training=self.training
            )
            value_states = quantize_kv_per_token(
                value_states,
                num_bits=self.quant_config.KV_bit,
                clip_val=self.quant_config.gradclip if self.quant_config.gradclip != (0, 0) else None,
                training=self.training
        ...
        if (past_key_values is not None and not is_cross_attention) or (
            ...
            # Note: quantized K/V are cached, so scaling factors are implicitly stored in the values
...
class GPT2Block(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx=None, quant_config: quantConfig = None):
        super().__init__()
        ...
        if config.add_cross_attention:
            self.crossattention = GPT2Attention(config=config, is_cross_attention=True, layer_idx=layer_idx, quant_config=quant_config)
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config, quant_config: quantConfig = None):
        ...
        self.h = nn.ModuleList([GPT2Block(config, layer_idx=i, quant_config=quant_config) for i in range(config.num_hidden_layers)])
...
class GPT2LMHeadModel(GPT2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, quant_config: quantConfig = None):
        super().__init__(config)
        self.quant_config = quant_config
        self.transformer = GPT2Model(config, quant_config=quant_config)
